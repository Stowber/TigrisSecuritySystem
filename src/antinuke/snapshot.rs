...
use serenity::all::{ChannelType, CreateChannel, CreateRole, GuildId, Permissions};
...
impl<'a> DiscordApi for SerenityApi<'a> {
    ...
    async fn create_role(&self, guild_id: u64, role: &RoleSnapshot) -> Result<()> {
        ...
        self.http
            .create_role(GuildId::new(guild_id), builder)
            .await?;
        Ok(())
    }
    ...
    async fn create_channel(&self, guild_id: u64, channel: &ChannelSnapshot) -> Result<()> {
        ...
        self.http
            .create_channel(GuildId::new(guild_id), builder)
            .await?;
        Ok(())
    }
}
