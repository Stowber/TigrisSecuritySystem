...
impl Default for IdgMode {
    fn default() -> Self { IdgMode::Monitor }
}
...
impl Default for IdgPreset {
    fn default() -> Self { IdgPreset::Balanced }
}
...
fn default_thresholds() -> IdgThresholds { IdgConfig::default().thresholds }
fn default_weights() -> IdgWeights { IdgConfig::default().weights }
...
impl Default for IdgConfig {
    fn default() -> Self {
        Self {
            ...
            thresholds: IdgThresholds { watch: 30, block: 60 },
            weights: IdgWeights {
                nick_token: 25,
                nick_regex: 35,
                avatar_hash: 40,
...
impl IdGuard {
    ...
    pub async fn check_user(&self, input: &IdgInput) -> IdgReport {
        let mut cfg = self.cfg_cache.get(&input.guild_id).map(|e| e.clone()).unwrap_or_default();
        ...
        if !norm_joined.is_empty() {
            ...
            if !allowed_by_rule {
                ...
                for hit in guard.iter().filter(|r| r.action == RuleAction::Deny).filter(|r| r.matches(&norm_joined, &lowered)) {
                    let w = match hit.kind {
                        RuleKind::Regex => cfg.weights.nick_regex,
                        RuleKind::Token => cfg.weights.nick_token,
                    };
                    signals.push(IdgSignal {
                        kind: if hit.kind == RuleKind::Regex { IdgSignalKind::NickRegex } else { IdgSignalKind::NickToken },
                        weight: w,
        ...
        if let Some(url) = &input.avatar_url {
            if let Ok(Some(h)) = fetch_and_ahash(url).await {
                avatar_hash = Some(h);
                let deny = self
                    .avatar_deny
                    .get(&input.guild_id)
            ...
            // OCR/NSFW stuby (utrzymane jako placeholdery)
            if let Some(_txt) = ocr_from_url_stub(url).await {
                // opcjonalnie: dodać heurystyki na podstawie OCR
            }
            if let Some(true) = nsfw_from_url_stub(url).await {
                signals.push(IdgSignal {
                    kind: IdgSignalKind::AvatarNSFW,
                    weight: cfg.weights.avatar_nsfw,
                    detail: "nsfw detector (stub)".into(),
                });
            }
        }
        ...
        IdgReport { score: score_u8, verdict, signals: s_sorted, explain, avatar_hash }
    }
    ...
    pub async fn log_review_embed(
        ...
        if let Some(nick_preview) = input.username.as_ref().or(input.display_name.as_ref()).or(input.global_name.as_ref()) {
            let tokens = tokens_for_buttons(nick_preview);
            let mut tokens_added = 0usize;
            for t in tokens {
                let patt = sanitize_custom_id(&t, 40);
                if patt != "_" {
                    buttons.push(CreateButton::new(format!("idg_allow_nick:{}", patt)).label(format!("Allow „{}”", clamp_label(&t))).style(ButtonStyle::Secondary));
                    buttons.push(CreateButton::new(format!("idg_deny_nick:{}", patt)).label(format!("Deny „{}”", clamp_label(&t))).style(ButtonStyle::Danger));
                    tokens_added += 1;
                    if tokens_added >= 4 { break; } // max 4 tokeny
                }
            }
        }
        ...
        if let Some(h) = report.avatar_hash {
            ...
            buttons.push(CreateButton::new(format!("idg_allow_avat:{}", hid)).label("Allow Avatar").style(ButtonStyle::Secondary));
            buttons.push(CreateButton::new(format!("idg_deny_avat:{}", hid)).label("Deny Avatar").style(ButtonStyle::Danger));
        }
        ...
        if !components.is_empty() { msg = msg.components(components); }
...
impl IdGuard {
    async fn on_cmd_idguard(&self, ctx: &Context, app: &AppContext, cmd: &CommandInteraction) -> Result<()> {
        maybe_ensure_tables(self.db()).await;

        // wymóg: staff
        let env = app.env();
        let staff = env_roles::staff_set(&env);
        let allowed = cmd.member.as_ref().map(|m| m.roles.iter().any(|r| staff.contains(&r.get()))).unwrap_or(false);
        if !allowed {
            let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                CreateInteractionResponseMessage::new().content("Brak uprawnień.").ephemeral(true)
            )).await;
            return Ok(());
        }
        ...
        let gid = if let Some(g) = cmd.guild_id {
            ...
            let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                CreateInteractionResponseMessage::new().content("Użyj na serwerze.").ephemeral(true)
            )).await;
            return Ok(());
        };
        ...
        match sub.as_deref() {
            Some("setup") => {
                let idg = self;
                let mut cfg = idg.cfg_cache.get(&gid.get()).map(|c| c.clone()).unwrap_or_default();
                cfg.enabled = true;
                cfg.mode = IdgMode::Monitor;
                cfg.preset = IdgPreset::Balanced;
                cfg.thresholds = IdgThresholds { watch: 30, block: 60 };
                cfg.weights = IdgWeights { nick_token: 25, nick_regex: 35, avatar_hash: 40, avatar_ocr: 30, avatar_nsfw: 50 };
                cfg = sanitize_cfg(cfg);
            ...
            Some("preset") => {
                ...
                match preset {
                    IdgPreset::Lenient => {
                        cfg.thresholds = IdgThresholds { watch: 40, block: 75 };
                        cfg.weights    = IdgWeights { nick_token: 20, nick_regex: 25, avatar_hash: 35, avatar_ocr: 25, avatar_nsfw: 50 };
                    }
                    IdgPreset::Balanced => {
                        cfg.thresholds = IdgThresholds { watch: 30, block: 60 };
                        cfg.weights    = IdgWeights { nick_token: 25, nick_regex: 35, avatar_hash: 40, avatar_ocr: 30, avatar_nsfw: 50 };
                    ...
                    IdgPreset::Strict => {
                        cfg.thresholds = IdgThresholds { watch: 20, block: 45 };
                        cfg.weights    = IdgWeights { nick_token: 30, nick_regex: 40, avatar_hash: 45, avatar_ocr: 35, avatar_nsfw: 55 };
                    }
                }
                cfg = sanitize_cfg(cfg);
                idg.cfg_cache.insert(gid.get(), cfg.clone());
                ...
                let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                    CreateInteractionResponseMessage::new().content(format!("Preset ustawiony: **{:?}**.", preset)).ephemeral(true)
                )).await;
            }
            Some("mode") => {
                let choice = sval.unwrap_or_else(|| "monitor".into());
                let mode = if choice == "auto" { IdgMode::Auto } else { IdgMode::Monitor };
                let idg = self;
                let mut cfg = idg.cfg_cache.get(&gid.get()).map(|c| c.clone()).unwrap_or_default();
                cfg.mode = mode;
                cfg = sanitize_cfg(cfg);
                ...
                let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                    CreateInteractionResponseMessage::new().content(format!("Tryb: **{:?}**.", mode)).ephemeral(true)
                )).await;
            }
            _ => {
                let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                    CreateInteractionResponseMessage::new().content("Nieznana subkomenda.").ephemeral(true)
                )).await;
            }
        }
    ...
    async fn on_cmd_teach(&self, ctx: &Context, app: &AppContext, cmd: &CommandInteraction) -> Result<()> {
        maybe_ensure_tables(self.db()).await;
        ...
        let action = match sub.as_deref() {
            ...
            _ => {
                let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                    CreateInteractionResponseMessage::new().content("Użyj `allow` lub `deny`.").ephemeral(true)
                )).await;
                return Ok(());
            }
        };
        ...
        let reason= params.get("reason").cloned().unwrap_or_else(|| "admin teach".into());

        // Wzajemnie wykluczaj nick i avatar, aby uniknąć częściowej obsługi
        if nick.is_some() && avatar.is_some() {
            let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
                CreateInteractionResponseMessage::new().content("Podaj **albo** `nick`, **albo** `avatar` (nie oba naraz).").ephemeral(true)
            )).await;
            return Ok(());
        }
        ...
        if let Some(nick_text) = nick {
            ...
            let list = self.nick_rules
                .entry(gid.get())
                .or_insert_with(|| Arc::new(RwLock::new(Vec::new())))
                .clone();
            {
                let mut guard = list.write().await;
                guard.retain(|r| !(r.action == rule.action && r.kind == rule.kind && r.pattern == rule.pattern));
        ...
        let _ = cmd.create_response(&ctx.http, CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new().content("Podaj `nick` lub `avatar`.").ephemeral(true)
        )).await;

        Ok(())
    }

    async fn on_btn_allow_deny_nick(&self, ctx: &Context, app: &AppContext, i: &ComponentInteraction, allow: bool) {
        let env = app.env();
        if !ensure_staff_ephemeral(ctx, &env, i).await { return; }

        let _ = i.create_response(&ctx.http, CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new().content(if allow { "Zapisuję allow…" } else { "Zapisuję deny…" }).ephemeral(true)
        )).await;

        let Some(gid) = i.guild_id else {
            let _ = i.edit_response(&ctx.http, EditInteractionResponse::new().content("Tylko na serwerze.")).await;
            return;
        };

        let patt = i.data.custom_id.splitn(2, ':').nth(1).unwrap_or("").to_string();
        let (kind, pat) = parse_pattern(&patt);
        let action = if allow { RuleAction::Allow } else { RuleAction::Deny };
        let rule = match NickRule::new(action, kind, pat.clone(), "button") {
            Ok(r) => r,
            Err(e) => {
                let _ = i.edit_response(&ctx.http, EditInteractionResponse::new().content(format!("Błąd: {}", e))).await;
                return;
            }
        };
        if let Err(e) = upsert_nick_rule(self.db(), gid.get(), &rule).await {
        ...
        let _ = i.edit_response(&ctx.http, EditInteractionResponse::new().content(
            format!("OK: {:?} {:?} `{}`", action, kind, pat)
        )).await;
    }

    async fn on_btn_allow_deny_avatar(&self, ctx: &Context, app: &AppContext, i: &ComponentInteraction, allow: bool) {
        let env = app.env();
        if !ensure_staff_ephemeral(ctx, &env, i).await { return; }
        ...
        let action = if allow { RuleAction::Allow } else { RuleAction::Deny };
        if let Err(e) = upsert_avatar_hash_deny_allow(self.db(), gid.get(), h, action, "button").await {
        ...
        let _ = i.edit_response(&ctx.http, EditInteractionResponse::new().content(
            format!("OK: {:?} avatar `{:016x}`", action, h)
        )).await;
    }
}
...
impl NickRule {
    ...
    fn matches(&self, text_raw: &str, text_lower: &str) -> bool {
        match self.kind {
            RuleKind::Token => {
                let pat = self.pattern_lower.as_ref().map(|s| s.as_str()).unwrap_or("");
                contains_token_cached(text_lower, pat)
            }
            RuleKind::Regex => self.compiled.as_ref().map(|r| r.is_match(text_raw)).unwrap_or(false),
        }
    }
}
...
fn clamp_label(s: &str) -> String { clamp_chars(s, 40) }
...
fn tokens_for_buttons(s: &str) -> Vec<String> {
    ...
    for ch in s.chars() {
        if ch.is_alphanumeric() {
            ...
            if cur.chars().count() >= 3 { tokens.push(cur.clone()); }
            cur.clear();
        }
    }
    if cur.chars().count() >= 3 { tokens.push(cur); }
...
fn collect_names(u: &Option<String>, d: &Option<String>, g: &Option<String>) -> Vec<String> {
    let mut v = Vec::new();
    if let Some(x) = u { v.push(x.clone()); }
    if let Some(x) = d { v.push(x.clone()); }
    if let Some(x) = g { v.push(x.clone()); }
    v
}
...
async fn fetch_and_ahash(url: &str) -> Result<Option<u64>> {
...
async fn ocr_from_url_stub(_url: &str) -> Option<String> {
    None
}
async fn nsfw_from_url_stub(_url: &str) -> Option<bool> {
    None
}
...
async fn ensure_tables(db: &Pool<Postgres>) -> Result<()> {
    ...
    // indeksy pomocnicze
    let _ = sqlx::query(
        r#"CREATE INDEX IF NOT EXISTS idx_idg_rules_guild ON tss.idg_rules(guild_id)"#,
    )
    .execute(db)
    .await?;
    let _ = sqlx::query(
...
async fn load_rules_db(db: &Pool<Postgres>, guild_id: u64) -> Result<Vec<NickRule>> {
    ...
    for r in rows {
        ...
        let action = if action_s.eq_ignore_ascii_case("allow") { RuleAction::Allow } else { RuleAction::Deny };
        let kind = if kind_s.eq_ignore_ascii_case("regex") { RuleKind::Regex } else { RuleKind::Token };
...
async fn upsert_nick_rule(db: &Pool<Postgres>, guild_id: u64, rule: &NickRule) -> Result<()> {
    ...
    .bind(guild_id as i64)
    .bind(match rule.action { RuleAction::Allow => "allow", RuleAction::Deny => "deny" })
    .bind(match rule.kind { RuleKind::Token => "token", RuleKind::Regex => "regex" })
    .bind(&rule.pattern)
    .bind(&rule.reason)
    .execute(db)
    .await?;
...
async fn upsert_avatar_hash_deny_allow(
    ...
    match action {
        ...
        RuleAction::Allow => {
            ...
            let _ = sqlx::query("DELETE FROM tss.idg_avatar_hash_deny WHERE guild_id=$1 AND hash=$2")
                .bind(guild_id as i64)
                .bind(hash as i64)
                .execute(db)
                .await?;
...
fn extract_teach_params(options: &[CommandDataOption]) -> (Option<String>, HashMap<String, String>) {
    if let Some(op) = options.first() {
        if let CommandDataOptionValue::SubCommand(params) = &op.value {
            let mut out: HashMap<String, String> = HashMap::new();
            for p in params {
                match (p.name.as_str(), &p.value) {
                    ("nick",   CommandDataOptionValue::String(s))   => { out.insert("nick".into(), s.clone()); }
                    ("avatar", CommandDataOptionValue::String(s))   => { out.insert("avatar".into(), s.clone()); }
                    ("reason", CommandDataOptionValue::String(s))   => { out.insert("reason".into(), s.clone()); }
                    _ => {}
                }
            }
...
async fn ensure_staff_ephemeral(ctx: &Context, env: &str, i: &ComponentInteraction) -> bool {
    let ok = i.member.as_ref().map(|m| is_staff_member_roles(env, &m.roles)).unwrap_or(false);
    if !ok {
...
pub fn sanitize_cfg(mut cfg: IdgConfig) -> IdgConfig {
    ...
    // wagi
    fn clamp_w(v: i32) -> i32 { v.clamp(0, 100) }
